...
public class CoreServiceImpl implements CoreService {
    ...
    @Autowired
    protected MsgTextHandler msgTextHandler;
    @Autowired
    protected MsgImageHandler msgImageHandler;
    @Autowired
    protected MsgVideoHandler msgVideoHandler;
    @Autowired
    protected MsgVoiceHandler msgVoiceHandler;
    @Autowired
    protected LocationHandler locationHandler;
    @Autowired
    protected UnsubscribeHandler unsubscribeHandler;
    @Autowired
    protected MenuClickHandler menuClickHandler;
    @Autowired
    protected MenuViewHandler menuViewHandler;
    ...
    public void refreshRouter() {
        ...
        // 自定义菜单click事件
        newRouter.rule().async(false).msgType(WxConsts.XML_MSG_EVENT)
                .event(WxConsts.BUTTON_CLICK).handler(this.menuClickHandler)
                .end();

        // 自定义菜单view事件
        newRouter.rule().async(false).msgType(WxConsts.XML_MSG_EVENT)
                .event(WxConsts.BUTTON_VIEW).handler(this.menuViewHandler)
                .end();
     ...
     * @Title WxMpXmlOutMessage
     * @Decription 接受用户消息并处理
     * @author zhangtianyu
     * @Date 2017/1/12
     */
    @Override
