...
//                    .setHttpProxy(new HttpHost("58.247.89.210", 9999))
    ...
    public void process(Page page) {
        ...
        map.put("内蒙古旅游局", "内蒙古");
        map.put("乐游上海", "上海");
//        page.addTargetRequests(page.getHtml().xpath("//div[@class='pa']/div/a[1]").links().all());
        page.addTargetRequests(page.getHtml().xpath("//table/tbody/tr/td[2]/a[1]").links().all());
////        page.addTargetRequests(page.getHtml().xpath("//div[@class='pms']/a[1]").links().regex("http://weibo\\.cn/.+?\\?filter=1").all());
////        page.addTargetRequests(page.getHtml().xpath("//div[@class='pa']/form/div/a[1]").links().regex("http://weibo\\.cn/.+?\\?page=[2-3]").all());
